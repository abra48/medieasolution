...
import { createIssue, type ActionResult } from "@/app/actions/admin";

type Platform = { id: string; name: string };

export function IssueForm({ platforms }: { platforms: Platform[] }) {
  ...
  return (
    ...
    >
      <div className="px-5 py-4 border-b border-border-default flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-text-primary">Tambah Kendala</h3>
      </div>
      ...
      <form action={action} className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="block text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-1.5">Platform *</label>
            <select name="platform_id" required className="input !py-2 text-sm">
          ...
          <div>
            <label className="block text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-1.5">Nama Kendala *</label>
            <input name="issue_name" required placeholder="e.g., Akun Terblokir" className="input !py-2 text-sm" />
          </div>
          <div>
